...
분석 진행 후 실제 매수 매도 신호를 리턴하는 CLASS
...
class HolicAgent:
    ...
    def __init__(self,  environment, min_trading_unit=1, max_trading_unit=2,
        ...
        #self.kiwoom_agent = Kiwoom.Kiwoom()
        self.modelholicstock = modelholicstock.ModelHolicStock()
    ...
    def decide_action(self, policy_network, sample, epsilon):
        '''
        입력 epsilon에 따라 무작위 탐험을 할지 정책신경망을 통해 행동할지 결정
        '''
        confidence = 0.
        # 탐험 결정
    ...
    def act(self, action, confidence):
        ...
        :param action:  탐험 또는 정책신경망에서 결정한 매수 0 매도 1
        :param confidence: 정책신경망에서 결정한 경우 softmax 확률값
        :return:
        '''
        if not self.validate_action(action):
            action = HolicAgent.ACTION_HOLD
        ...
        # 홀딩
        elif action == HolicAgent.ACTION_HOLD:
    ...
    def run(self):
        ...
        #1. 매수 종목 확인. ( DB에 저장되어 있는 active 상태의 대상 종목 조회)
        self.buy_list = modelholicstock.read_my_stock('buy')
        print ('#### buy list : ' +  self.buy_list )

        self.sell_list = modelholicstock.read_my_stock('sell')
        print('#### sell list : ' + self.sell_list)

        #2. 매수 가능 금액 확인 및 대상 별 매수 금액 결정
        ...
        buyResult = []
        sellResult = []
        cnt = len(self.buy_list) + len(self.sell_list)
        ...
        for i in range(cnt):
            ...
            try:
                if stocks[5].rstrip() == '매수전':
                    self.kiwoom_agent.sendOrder("자동매수주문", "0101", account, 1, code, int(qty), int(price), hogaTypeTable[hoga],
                                          "")

                    # 주문 접수시
                    if self.kiwoom_agent.orderNo:
                        buyResult += automatedStocks[i].replace("매수전", "매수주문완료")
                        self.kiwoom_agent.orderNo = ""
                    # 주문 미접수시
                    else:
                ...
                # 참고: 해당 종목을 현재도 보유하고 있다고 가정함.
                    ...
                    self.kiwoom_agent.sendOrder("자동매도주문", "0101", account, 2, code, int(qty), int(price), hogaTypeTable[hoga],
                                          "")
        ...
        # 잔고및 보유종목 디스플레이 갱신
        self.inquiryBalance()
        #self.update_mystock_list(mystock_list)
